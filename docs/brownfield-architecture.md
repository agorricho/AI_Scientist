...
| **Vector Store (Fallback)** | ChromaDB | 1.4.1 | Embedded, in-memory, NOT persisted |
| **Vector Store (Primary)** | Qdrant | v1.16.3 (server) / 1.16.2 (client) | Persistent, Docker container on localhost:6333 |
...
| **Container Runtime** | Docker Engine | 29.1.3 | WSL2, systemd required |
| **Container Compose** | Docker Compose | v5.0.2 | Via docker-compose-plugin |
...
| **ChromaDB not persisted** | Re-indexes on every run | Slow for large corpora, replace with Qdrant | HIGH |
...
| **Ollama** | LLM inference (Docker) | REST localhost:11435 (maps to 11434 inside container) | `causal_extractor.py`, `query_rephraser.py` |
| **Qdrant** | Vector database (Docker) | REST localhost:6333, gRPC localhost:6334 | `causal_extractor.py` |

### Docker Compose Infrastructure

Both Ollama and Qdrant run as Docker containers managed by `docker-compose.yml` in `rag_prototype/`.

**Compose file:** `rag_prototype/docker-compose.yml`
...
### Ollama LLM Server (Docker Container)

| Property | Value |
|----------|-------|
| **Container Name** | `ollama` |
| **Image** | `ollama/ollama:0.15.6` |
| **Mode** | CPU-only (no GPU passthrough) |
| **REST API (host)** | `http://localhost:11435` |
| **REST API (internal)** | `http://ollama:11434` (via `rag-net` bridge) |
| **Persistent Storage** | Docker volume `ollama_data` at `/root/.ollama` |
| **Health Check** | `curl http://localhost:11435/api/version` |
| **LLM Model** | `qwen2.5:14b-instruct` (~9GB) |
| **Embedding Model** | `nomic-embed-text` (~274MB, 768-dim vectors) |
...
| **Persistent Storage** | Docker volume `qdrant_data` at `/qdrant/storage` |
| **Health Check** | `curl http://localhost:6333/healthz` |
| **Collection Name** | `papers_vectors` |
...
**Note:** Port mapping is `11435:11434`. The codebase default (`11435`) matches the Docker compose host port. Ollama runs on its standard port `11434` inside the container.
...
# Start Ollama + Qdrant containers
docker compose up -d
...
```bash
# Start Docker infrastructure (Ollama + Qdrant)
cd rag_prototype && docker compose up -d
...
# Pull a new model into Ollama container
